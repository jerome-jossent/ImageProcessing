...
{
    public static MQTT_JJ _mqtt_client;
    public void Awake()
    {
    ...
    public class MQTTMessage_JJ
    {
        ...
        internal int? _GetDataAsInt32()
        {
            if (int.TryParse(_GetDataAsString(), out int value)) return value;
            return null;
        }
        internal long? _GetDataAsLong()
        {
            if (long.TryParse(_GetDataAsString(), out long value)) return value;
            return null;
        }
        internal float? _GetDataAsFloat()
        {
            if (float.TryParse(_GetDataAsString(), out float value)) return value;
            return null;
        }
        internal double? _GetDataAsDouble()
        {
            if (double.TryParse(_GetDataAsString(), out double value)) return value;
            return null;
        }
    ...
    internal void _UnSubscribeTopic(string topic)
    {
        if (topic == "") return;
        if (topics_subscribed_unityevent.ContainsKey(topic))
        {
